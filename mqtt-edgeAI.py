...
class MqttDashboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Blickfeld: MQTT Intrusion Monitor (With Jitter, Throughput & Throttle Loss)")
    ...
    def _build_ui(self):
        ...
        tk.Label(ctrl_frame, text="⚙️ Rate Limit (FPS):", font=("Arial", 10, "bold"), bg="#252538", fg="white").pack(side=tk.LEFT, padx=5)
    ...
    def _ui_consumer_tick(self):
        ...
        if latest_mqtt:
                ...
                if now <= self.bench_end_time:
                    ...
                    if len(self.bench_tot_latencies) > 0:
                        ...
                        summary = (
                            ...
                            f" ├── Avg Total Latency : {avg_tot:.2f} ms\n"
                            f" ├── Jitter (Broker Delivery & OS Queue Standard Deviation σ): ±{tot_jitter:.2f} ms\n"
                            f" ├── Throughput (JSON Text Bandwidth & Effective Render Rate): {kbps:.2f} KB/s ({fps:.1f} FPS)\n"
                            f" └── Packet Loss (Frames Discarded by Client Rate Limiter)   : {self.bench_frames_throttled} frames ({drop_rate:.1f}% drop)\n"
                            f"=========================================================================================="
                        )
            ...
            if sensor_epoch > 0:
                ...
                lat_str = f"Total: {total_ms:.1f}ms"
