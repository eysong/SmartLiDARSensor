...
class LidarDashboardApp:
  ...
  def _ui_consumer_tick(self):
    ...
    if latest_mqtt and self.clock_offset is not None:
      ...
      # extract MQTT creation timestamp from JSON
      raw_ts = payload.get("timestamp") or payload.get("time") or 0.0
      try:
        val = float(raw_ts)
        # normalize nanoseconds or milliseconds into standard Unix seconds
        sensor_epoch = val / 1e9 if val > 1e16 else (val / 1e3 if val > 1e10 else val)
      except (ValueError, TypeError):
        sensor_epoch = 0.0

      # apply the gRPC-derived hardware clock offset to the MQTT payload!
      if sensor_epoch > 0:
        ...
        lat_str = "UNKNOWN (No timestamp in payload)"

      incoming_intrusions = []

      # look for a list of objects or intruders in the native JSON payload
      objs = payload.get("objects", payload.get("intruders", []))
      
      if isinstance(objs, dict):
        obj_list = objs.values()
      elif isinstance(objs, list):
        obj_list = objs
      else:
        obj_list = [payload]  # fallback: maybe the payload is the object itself

      for obj in obj_list:
        if not isinstance(obj, dict):
          continue

        # look for intrusion flags anywhere in the object or parent payload
        if (
