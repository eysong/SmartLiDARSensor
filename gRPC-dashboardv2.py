...
class LidarDashboardApp:
  ...
  def __init__(self, root):
    self.root = root
    self.root.title(
        "Blickfeld QB2 Architecture Benchmark: Edge-AI vs Raw Cloud Stream"
    )
    ...
    # clock auto-zero calibration state
    self.calib_deltas = []
  ...
  def _build_ui(self):
    ...
    self.notebook.add(self.edge_tab, text="  ⚡ PAGE 1: EDGE-AI TELEMETRY  ")
    self.notebook.add(self.raw_tab, text="  📡 PAGE 2: RAW DATA & BENCHMARK  ")
  ...
  def _build_edge_tab(self):
    ...
    subjects_frame = tk.LabelFrame(
        self.edge_tab,
        text=f" Edge-AI Tracked Subjects inside '{TARGET_ZONE}' ",
        bg="#1e1e2e",
        fg="#cdd6f4",
        font=("Arial", 11, "bold"),
    )
    ...
    perf_frame = tk.LabelFrame(
        self.edge_tab,
        text=" Edge-AI Time-to-Insight Analytics (~120 Bytes/Frame) ",
        bg="#1e1e2e",
        fg="#cdd6f4",
        font=("Arial", 11, "bold"),
    )
  ...
  def _build_raw_tab(self):
    ...
    pc_frame = tk.LabelFrame(
        self.raw_tab,
        text=" Unprocessed 3D Laser Stream (Subsampled for Display) ",
        bg="#1e1e2e",
        fg="#cdd6f4",
        font=("Arial", 11, "bold"),
    )
    ...
    bench_frame = tk.LabelFrame(
        self.raw_tab,
        text=" Timed Raw Stream Network Benchmarker ",
        bg="#1e1e2e",
        fg="#cdd6f4",
        font=("Arial", 11, "bold"),
    )
  ...
  def log_message(self, text_widget, msg):
    """Thread-safe logging helper targeting specific tab text boxes."""
  ...
  def _start_timed_benchmark(self):
    """Initiates a timed network latency benchmark on the raw point cloud."""
  ...
  def _update_smart_ui(self, is_alarm, display_list, xyz_data):
    """Only repaints UI components on the currently visible tab."""
    current_tab = self.notebook.index(self.notebook.select())

    # --- IF ON PAGE 1 (EDGE-AI) ---
    if current_tab == 0:
  ...
  def _grpc_objects_producer(self):
    ...
    try:
      ...
      with blickfeld_qb2.Channel(
        ...
        self.packet_queue.put(("EDGE_LOG", f"ONLINE: Connected to Edge-AI"))

        for response in service.stream():
          self.packet_queue.put(("OBJECTS", time.time(), response.to_dict()))
    except Exception as e:
      self.packet_queue.put(("EDGE_LOG", f"ERROR: {str(e)}"))
    ...
    try:
      ...
      with blickfeld_qb2.Channel(
        ...
        self.packet_queue.put(
            ("RAW_LOG", f"ONLINE: Connected to Raw Point Cloud stream")
        )
  ...
  def _ui_consumer_tick(self):
    ...
    if latest_pc:
      ...
      # we calibrate using the raw stream because it bypasses Edge compute delay!
      if self.clock_offset is None:
        ...
        if len(self.calib_deltas) >= 30:
          ...
          self.clock_offset = fastest_raw_gap - 0.003 # 3ms unmanaged switch baseline
          self.log_message(
    ...
    # process edge-AI objects
    if latest_objects and self.clock_offset is not None:
      _, wire_arrival_time, frame_data = latest_objects
      now = time.time()
      sensor_epoch = self._parse_sensor_epoch(frame_data)

      # edge latency includes Network Transit + On-Board Compute Time
      if sensor_epoch > 0:
      ...
      if len(incoming_intrusions) > 0 and (
        ...
        log_msg = (
            f"INTRUSION | Edge-AI Pipeline Latency: {lat_str} | Active"
